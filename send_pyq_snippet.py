#!/usr/bin/env python3
"""
Send a PyQ markdown snippet email, rotating through sections sequentially or by explicit index.
"""
...
def load_sections(path: pathlib.Path) -> List[Tuple[str, str]]:
    ...
    if not sections:
        raise RuntimeError("No sections found in markdown.")
    return sections
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Send PyQ markdown snippet email.")
    parser.add_argument(
        "--index",
        type=int,
    ...
    print(f"Email sent successfully for {heading}.")
